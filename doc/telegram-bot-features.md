...
| **OMX interactive question artifacts** | ✅ | Durable root/session `omx.question/v1` records are rendered as a separate mutable Telegram artifact with inline option buttons; split-pane renderers remain inside the parent bound tmux window rather than becoming bindable surfaces; button answers and allowed free-text `Other` replies update the state record, bridge back to the recorded tmux return pane, and close the temporary question pane |
