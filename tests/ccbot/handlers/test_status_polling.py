...
async def test_status_polling_projects_omx_workflow_status_from_state(
    ...
    mock_window.pane_current_command = "node"
    state_path = tmp_path / ".omx" / "state" / "sessions" / "abc" / "ultragoal-state.json"
        ...
        json.dumps(
            {
                ...
                "current": 1,
                "total": 1,
                "goal_id": "G001-implement-omx-workflow-status-reader",
                "current_unit_summary": "Implement OMX workflow status reader and renderer",
            }
        )
    )
...
@pytest.mark.asyncio
async def test_status_polling_suppresses_unrelated_omx_state_for_same_cwd(
    mock_bot: AsyncMock, tmp_path: Path
):
    window_id = "@5"
    mock_window = MagicMock()
        ...
        json.dumps(
            {
                "mode": "ultragoal",
                "status": "in_progress",
                "tmux_window_id": "@other",
                "current_unit_summary": "Wrong window",
            }
        )
    )
